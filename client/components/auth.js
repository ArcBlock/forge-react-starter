...
export default function Auth({ onClose, onSuccess, action, prefix, messages, checkInterval }) {
  ...
  useInterval(
    ...
    token && !['succeed', 'error'].includes(status) ? checkInterval : null
  );
      ...
      <Container>
        ...
        <div className="container">
          ...
          <div className="qrcode">
            <QRCode size={240} renderAs="svg" level="M" value={refresh ? refresh.url : state.value.url} />
          </div>
  ...
  if (status === 'error') {
    return (
      <Container>
        ...
        <div className="container">
          <div className="tip tip--shout">Oops</div>
          <div className="tip tip--confirm">We encountered some errors when processing the request</div>
        </div>
      </Container>
    );
  }
